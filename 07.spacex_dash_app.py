...
# Questo non serve ma ce lo lascio lo stesso
# fig=px.pie(spacex_df[spacex_df['class']==1].groupby('Launch Site').count(),
#             values='class',
#             names=spacex_df[spacex_df['class']==1].groupby('Launch Site').count().index,
...
def get_pie_chart(entered_site):
    ...
    if entered_site=='ALL':
        filtered_df=spacex_df[spacex_df['class']==1].groupby('Launch Site').count()
        # print('ALL')
        fig=px.pie(filtered_df,
            values='class',
            names=filtered_df.index,
            title='Total Success Launches By All Sites',hole=.3)
        return fig
    else:
...
def update_scatter(entered_site, payload_range):
    if entered_site == 'ALL':
        # Filter data with payload range
...
5. Which F9 Booster version (v1.0, v1.1, FT, B4, B5, etc.) has the highestlaunch success rate?
    B5 but he did only one launch. It is better B4
'''